...
explore: advanced_coalesce_solution {
  ...
  from: accounts
  label: "Normalized Schema (Advanced)"
  description: "The most robust solution. Prevents fan-out and allows dimensions from associated tables to be used correctly with measures from any fact table (Managers, Products, Pageviews, Orders)."
  ...
  join: associated_account {
    ...
    # Look for an account name in any of the base fact tables.
    sql_on: ${associated_account.name} = COALESCE(
        ${managers_base.account_name},
        ${products_base.account_name},
        ${pageviews_base.account_name},
        ${orders_base.account_name}
      ) ;;
  }
}
